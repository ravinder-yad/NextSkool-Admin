...
import { HiOutlinePlus, HiOutlinePencil, HiOutlineTrash, HiOutlineAcademicCap } from 'react-icons/hi2';

const API_URL = 'http://localhost:5000/api/placements/internships';
...
const Internships = () => {
  ...
  const initialFormState = { title: '', company: '', duration: '', stipend: '', location: '', applyLink: '', lastDate: '' };
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <HiOutlineAcademicCap className="text-primary" /> Internships
          </h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage student internship opportunities</p>
        </div>
        <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 bg-primary hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm">
          <HiOutlinePlus size={18} /> Add Internship
        </button>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-800">
                ...
                <th className="py-4 px-6 text-sm font-semibold text-gray-600 dark:text-gray-300">Duration</th>
                <th className="py-4 px-6 text-sm font-semibold text-gray-600 dark:text-gray-300">Stipend</th>
              ...
              {loading ? (
                ...
                data.map((item) => (
                  <tr key={item._id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                    ...
                    <td className="py-4 px-6 text-sm text-gray-600 dark:text-gray-400">{item.duration}</td>
                    <td className="py-4 px-6 text-sm text-gray-600 dark:text-gray-400">{item.stipend}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-2xl shadow-2xl border border-gray-100 dark:border-gray-800 flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 dark:border-gray-800">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Add Internship</h2>
            </div>
            <div className="p-6 overflow-y-auto">
              <form id="record-form" onSubmit={handleSubmit} className="space-y-4 grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 dark:text-gray-300">Duration (e.g. 6 Months)</label>
                  <input type="text" name="duration" value={formData.duration} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg dark:bg-gray-800 dark:border-gray-700 dark:text-white" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 dark:text-gray-300">Stipend</label>
                  <input type="text" name="stipend" value={formData.stipend} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg dark:bg-gray-800 dark:border-gray-700 dark:text-white" />
                </div>
                <div>
                ...
                <div className="col-span-2">
                  <label className="block text-sm font-medium mb-1 dark:text-gray-300">Apply Link</label>
                  <input type="url" name="applyLink" value={formData.applyLink} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg dark:bg-gray-800 dark:border-gray-700 dark:text-white" />
                </div>
              </form>
            </div>
