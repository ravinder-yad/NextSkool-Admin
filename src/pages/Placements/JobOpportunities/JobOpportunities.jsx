...
import { HiOutlinePlus, HiOutlinePencil, HiOutlineTrash, HiOutlineBriefcase } from 'react-icons/hi2';

const API_URL = 'http://localhost:5000/api/placements/jobs';
...
const JobOpportunities = () => {
  ...
  const initialFormState = { title: '', company: '', experience: '', salary: '', location: '', applyLink: '', lastDate: '' };
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <HiOutlineBriefcase className="text-primary" /> Job Opportunities
          </h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage alumni and fresher job postings</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-800">
                ...
                <th className="py-4 px-6 text-sm font-semibold text-gray-600 dark:text-gray-300">Experience</th>
              ...
              {loading ? (
                ...
                data.map((item) => (
                  <tr key={item._id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                    ...
                    <td className="py-4 px-6 text-sm text-gray-600 dark:text-gray-400">{item.experience}</td>
                    <td className="py-4 px-6 text-sm text-gray-600 dark:text-gray-400">{item.location}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-2xl shadow-2xl border border-gray-100 dark:border-gray-800 flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 dark:border-gray-800">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Add Job Opportunity</h2>
            </div>
            <div className="p-6 overflow-y-auto">
              <form id="record-form" onSubmit={handleSubmit} className="space-y-4 grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 dark:text-gray-300">Experience (e.g. 0-2 yrs)</label>
                  <input type="text" name="experience" value={formData.experience} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg dark:bg-gray-800 dark:border-gray-700 dark:text-white" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 dark:text-gray-300">Salary Package</label>
                  <input type="text" name="salary" value={formData.salary} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg dark:bg-gray-800 dark:border-gray-700 dark:text-white" />
                </div>
                <div>
