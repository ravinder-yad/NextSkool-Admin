...
const RecentData = ({ latestStudents = [], latestPayments = [] }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      {/* Latest Students */}
      <div className="bg-white dark:bg-[#23273D] dark:shadow-none rounded-[20px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none border-none dark:border-transparent overflow-hidden flex flex-col">
        <div className="p-6 border-b border-gray-100/60 dark:border-transparent flex justify-between items-center">
          ...
          <table className="w-full text-left whitespace-nowrap">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-white/10">
              {latestStudents.map((s, i) => (
                <tr key={i} className="hover:bg-gray-50 dark:bg-white/5/50">
                  <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">{s.name}</td>
                  <td className="px-6 py-4 text-gray-600 dark:text-gray-300 dark:text-gray-300">{s.email}</td>
